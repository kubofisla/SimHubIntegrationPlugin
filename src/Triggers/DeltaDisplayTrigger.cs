    ...
    public class DeltaDisplayTrigger : PluginDynamicCommand, IDataTrigger
    {
        ...
        public DeltaDisplayTrigger() : base()
        {
            try
            {
                ...
                if (!SimHubData.Instance.Data.TryGetValue(EDataKey.SessionBestLiveDeltaSeconds, out var value))
                {
                ...
                this.AddParameter(nameof(DeltaDigits.Sign), "Delta time - sign", this.GroupName);
                this.AddParameter(nameof(DeltaDigits.SecondsWithDecimal), "Delta time - seconds", this.GroupName);
                this.AddParameter(nameof(DeltaDigits.Milliseconds), "Delta time - miliseconds", this.GroupName);
        ...
        protected override BitmapImage GetCommandImage(String actionParameter, PluginImageSize imageSize)
        {
            try
            {
                ...
                if (SimHubData.Instance.Data.TryGetValue(EDataKey.TargetTimeDelta, out var binding))
                {
                    if (binding.Value is TimeSpan timespan)
                    {
                        this._lastDeltaValue = timespan.TotalSeconds;
                    }
                    else if (binding.Value is Double doubleVal)
                    {
                        this._lastDeltaValue = doubleVal;
                    }
                }

                var deltaDisplay = DeltaDisplay.FromSeconds(this._lastDeltaValue);
